...
impl DataMemory {
    ...
    pub fn get_changed_memory(&mut self) -> ((i32, i32), (usize, usize), bool, bool) {
        // Save changed data
        let mut address = std::ops::RangeInclusive::new(
            (self.prev_memory_index.0 * 8) as usize,
        ...
        if self.prev_memory_index.0 < ((MAX_WORDS as u32 * 4) - 4).try_into().unwrap() {
            address = std::ops::RangeInclusive::new(
                (self.prev_memory_index.1 * 8) as usize,
...
impl Unit for DataMemory {
    ...
    fn execute(&mut self) {
        if self.has_address
            && self.has_write_data
            && self.has_mem_read_signal
            && self.has_mem_write_signal
        {
            if self.mem_write_signal {
                ...
                let address = std::ops::RangeInclusive::new(
                    (self.address * 8) as usize,
                    ((self.address * 8) + 31) as usize,
                );
