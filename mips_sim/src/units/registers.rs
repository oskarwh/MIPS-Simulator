...
impl Registers<'_>{
    ...
    pub fn execute(&mut self){
        if self.has_read1{
            ...
            self.alu.unwrap().receive(ALU_IN_1_ID, data.to_bitvec());
        ...
        if self.has_read2{
            ...
            self.mux_alu_src.unwrap().receive(MUX_IN_0_ID, data.to_bitvec());
            self.data_memory.unwrap().receive(DM_DATA_ID, data.to_bitvec());
