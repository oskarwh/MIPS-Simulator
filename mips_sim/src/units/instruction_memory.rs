    ...
    pub fn execute(&mut self) {
        if self.has_address {
            ...
            self.concater
                .as_mut()
                .unwrap()
                .receive(CONC_IN_1_ID, borrow.to_bitvec());

            //Send instruction to other units
            self.reg
                .as_mut()
                .unwrap()
                .receive(REG_READ_1_ID, self.current_instruction[21..26].to_bitvec());
            self.reg
                .as_mut()
                .unwrap()
                .receive(REG_READ_2_ID, self.current_instruction[16..21].to_bitvec());
            self.control
                .as_mut()
                .unwrap()
                .receive(CTRL_IN_ID, self.current_instruction[26..32].to_bitvec());
            self.alu_ctrl
                .as_mut()
                .unwrap()
                .receive(ALU_CTRL_IN_ID, self.current_instruction[0..6].to_bitvec());
            self.sign_extend
                .as_mut()
                .unwrap()
                .receive(ALU_CTRL_IN_ID, self.current_instruction[0..16].to_bitvec());
...
impl Unit for InstructionMemory<'_> {
    ...
    fn get_data(&self, input_id: u32) -> Word {
        match input_id {
            CONC_IN_1_ID => return self.to_concater.to_bitvec(),
            REG_READ_1_ID => return self.to_read_reg1.to_bitvec(),
            REG_READ_2_ID => return self.to_read_reg2.to_bitvec(),
            CTRL_IN_ID => return self.to_control.to_bitvec(),
            ALU_CTRL_IN_ID => return self.to_alu_ctrl.to_bitvec(),
            CTRL_IN_ID => return self.to_sign_extended.to_bitvec(),
            CTRL_IN_ID => return self.to_mux_branch.to_bitvec(),
        }
    }
